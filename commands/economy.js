const { sck, smd, getBuffer, tlang, prefix } = require('../lib')
...
smd(
 ...
 async (Aviator, msg, text, { isCreator }) => {
  ...
  if (mongoschemas == 'false') {
   return msg.reply('*🚦Economy* is not active in current group.')
  }
  if (!msg.isGroup) {
...
smd(
 {
  name: 'resetwallet',
  info: 'reset wallet of quoted user.',
  type: 'economy',
 ...
 async (Aviator, msg, text, { isCreator }) => {
  ...
  if (!users) {
   return msg.reply('Please give me user.')
  }
...
smd(
 ...
 async (Aviator, msg, text, { isCreator }) => {
  ...
  switch (value) {
   ...
   case '1':
    if (k > balance.wallet) {
     return msg.reply(`*_You need to pay 🪙100 to increase bank capacity ~ 1000 sp_*`)
    }
   ...
   case '2':
    if (k < balance.wallet) {
     return msg.reply(`*You need to pay 🪙1000 to increase bank capacity ~ 100000 sp*`)
    }
   ...
   case '3':
    if (k < balance.wallet) {
     return msg.reply(`You need to pay 🪙10000 to increase bank capacity ~ 1000 sp`)
    }
...
smd(
 ...
 async (Aviator, msg, text, { isCreator }) => {
  ...
  //  let users = msg.mentionedJid ? msg.mentionedJid[0] : msg.msg.contextInfo.participant || false;
  if (!text) {
   return msg.reply('Baka!! Provide the 💰amount you want to deposit!')
  }
  let d = parseInt(text)
  const deposit = await eco.deposit(msg.sender, 'Suhail', d)
  const balance = await eco.balance(msg.sender, 'Suhail')
  if (deposit.noten) {
   return msg.reply("You can't deposit what you don't have💰.")
  } //if user states more than whats in his wallet
...
smd(
 ...
 async (Aviator, msg, text, { isCreator }) => {
  ...
  if (value[0] === '') {
   return msg.reply(`Use ${prefix}transfer 100 @user`)
  }
  let user = msg.mentionedJid ? msg.mentionedJid[0] : msg.msg.contextInfo.participant || false
  if (!user) {
   return msg.reply('Please give me any user🤦‍♂️.')
  }
  ...
  let d = parseInt(word)
  if (!d) {
   return msg.reply('check your text plz u r using the command in a wrong way👀')
  }
  const balance = await eco.balance(user1, 'Suhail')
  let a = balance.wallet < parseInt(word)
  //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
  if (a == true) {
   return msg.reply('you dont have sufficient money to transfer👎')
  }
...
smd(
 ...
 async (Aviator, msg, text, { isCreator }) => {
  ...
  let users = msg.mentionedJid ? msg.mentionedJid[0] : msg.msg.contextInfo.participant || false
  if (!users) {
   return msg.reply('Please give me user to add money.')
  }
  await eco.give(users, 'Suhail', parseInt(text.split(' ')[0]))
  return await Aviator.bot.sendMessage(
   msg.chat,
   {
...
smd(
 ...
 async (Aviator, msg, text, { isCreator }) => {
  ...
  let users = msg.mentionedJid ? msg.mentionedJid[0] : msg.msg.contextInfo.participant || false
  if (!users) {
   return msg.reply('Please give me user to rob.')
  }
  const user1 = msg.sender
  const user2 = users
  const k = 1000
  ...
  if (k > balance1.wallet) {
   return msg.reply(`*☹️ You don't have enough money to pay incase you get caught*`)
  }
  if (k > balance2.wallet) {
   return msg.reply(`*Sorry, your victim is too poor 🤷🏽‍♂️ let go🫤.*`)
  }
  let tpy = random
  switch (random) {
   case 'ran':
...
smd(
 {
  name: 'withdraw',
  info: 'withdraw money from bank account.',
  type: 'economy',
 ...
 async (Aviator, msg, text, { isCreator }) => {
  ...
  if (!text) {
   return msg.reply('*Provide the amount💰 you want to withdraw💳!*')
  }
  const query = text.trim()
  const withdraw = await eco.withdraw(user, 'Suhail', query)
  if (withdraw.noten) {
   return msg.reply('*🏧 Insufficient fund in bank🫤*')
  } //if user states more than whats in his wallet
...
smd(
 ...
 async (Aviator, msg, text, { isCreator }) => {
  ...
  if (opp === 'left') {
   ...
   msg.reply(`Please provide direction(left,right,up,down).\nEg:- ${prefix}gamble 200 left`)
  }
  ...
  if (!text) {
   return msg.reply(`Example:  ${prefix}gamble 100 direction(left,right,up,down)`)
  }
  if (!value) {
   return msg.reply('*Please, specify the amount you are gambling with!*')
  }
  if (!opp) {
   return msg.reply('*Specify the direction you are betting on!*')
  }
  if (!gg) {
   return msg.reply('*Check your text please, You are using the command in a wrong way*')
  }
  if (g == false) {
   return msg.reply(`*You don't have sufficient 🪙 Diamond to gamble with*`)
  }
  if (a == true) {
   return msg.reply(`*Sorry ${msg.pushName}, you can only gamble with more than 🪙50.*`)
  }
  if (r == opp) {
   let give = await eco.give(user, 'Suhail', twice)
...
smd(
 ...
 async (msg, text) => {
  ...
  if (today.getDay() == 6 || today.getDay() == 5 || today.getDay() == 0) {
   if (text == 'help') {
    return msg.reply(`*1:* Use ${prefix}slot to play\n\n*2:* You must have 🪙100 in your wallet\n\n*3:* If you don't have money in wallet then 👛withdraw from your bank🏦\n\n*4:* If you don't have 🤑 money in your 🏦bank too then use economy features to 📈gain money`)
   }
   if (text == 'money') {
    return msg.reply(`*1:* Small Win --> +🪙20\n\n*2:* Small Lose --> -🪙20\n\n*3:* Big Win --> +🪙100\n\n*4:* Big Lose --> -🪙50\n\n*5:* 🎉 JackPot --> +🪙1000`)
   }
   const fruit1 = ['🥥', '🍎', '🍇']
   const fruit2 = ['🍎', '🍇', '🥥']
   const fruit3 = ['🍇', '🥥', '🍎']
   ...
   if (k > balance1.wallet) {
    return msg.reply(`You are going to be spinning on your wallet, you need at least 🪙100`)
   }
   ...
   if (f1 !== f2 && f2 !== f3) {
    ...
    msg.reply(`Do you understand what you are doing?`)
   }
  } else {
   msg.reply(`*You can only play this game during weekends*\n\n*🌿 Friday*\n*🎏 Saturday*\n*🎐 Sunday*`)
  }
 }
)
smd(
 ...
 async msg => {
  ...
  r_ban[17] = '6 : 5 : 1'
  r_ban[18] = '5 : 7 : 2'
  var p = Math.floor(Math.random() * 19)
  var q = Math.floor(Math.random() * 19)
  ...
  let ttt = k.split(':')
  var lol
  if (t[2] === tt[1] && tt[1] === ttt[0]) {
   lol = true
  }
  if (t[0] === tt[1] && tt[1] === ttt[2]) {
   lol = true
  }
  if (t[0] === tt[0] && tt[0] === ttt[0]) {
   lol = true
  }
  if (t[1] === tt[1] && tt[1] === ttt[1]) {
   lol = true
  }
  if (t[2] === tt[2] && tt[2] === ttt[2]) {
   lol = true
  }
  if (t[0] === tt[1] && tt[1] === ttt[2]) {
   lol = true

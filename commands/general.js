...
//---------------------------------------------------------------------------
smd({name: "dalle",alias : ['dall','dall-e'],info: "chat with an AI",type: "ai", use: '<Hii, Suhail Tech Info>',},
    async(Aviator, msg,text) =>{
  if ( Config.OPENAI_API_KEY=='' ||  !Config.OPENAI_API_KEY.startsWith('sk') ) return msg.reply('```You Dont Have OPENAI API KEY \nPlease Create OPEN API KEY from Given Link \nhttps://platform.openai.com/account/api-keys\nAnd Set Key in Heroku OPENAI_API_KEY Var```')
...
//---------------------------------------------------------------------------
smd({name: "rmbg",alias : ['removebg'],type: "ai",info: "Remove image Background." },
async(Aviator, msg, text) => {
...
///=============================================
smd({name: "uptime",alias: ["runtime"],info: "Tells runtime/uptime of bot.",type: "misc", filename: __filename},async(Suhail, msg, text) => {const upt = runtime(process.uptime());msg.reply(`*_Uptime of ${Config.botname}: ${upt}_*`);})
//---------------------------------------------------------------------------
//                  RREPOSITORY  COMMAND
//---------------------------------------------------------------------------
smd({

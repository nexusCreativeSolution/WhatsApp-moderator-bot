...
const { fetchJson, tlang, getBuffer, send, prefix, Config } = require('../lib')
...
Module(
 {
  name: 'find',
  type: 'misc',
  info: 'Finds info about song',
 },
 async (message, match) => {
  let mime = match.quoted.mtype
  if (!match.quoted) return match.reply(`Send/Reply audio with ${prefix}find`)
  if (!/audio/.test(mime)) return match.reply(`Send/Reply audio with ${prefix}shazam`)
  let buff = await match.quoted.download()
  let data = await shazam(buff)
  if (!data.status) return match.send(data)

  let search = await yts(data.title)
  let anu = search.videos[0]

  let info = `*Title : _${data.title}_*           
  ...
  await message.bot.sendMessage(match.chat, buttonMessage, { quoted: match })
 }
)
...
Module(
 ...
 async (Suhail, citel, text) => {
  try {
   ...
   return send(citel, '```Error While Fetching Snapshot```')
  }
 }
)
...
Module(
 {
  name: 'imdb',
  type: 'search',
  info: 'Sends image of asked Movie/Series.',
  use: '<text>',
 },
 async (Suhail, citel, text) => {
...
Module(
 {
  name: 'weather',
  type: 'search',
  info: 'Sends weather info about asked place.',
  use: '<location>',
 },
 async (Suhail, citel, text) => {
...
Module(
 {
  name: 'horo',
  type: 'search',
  info: 'Gives horoscope info of user.',
  use: '<sign>\n:Example: horo libra',
 },
 async (Suhail, citel, text) => {
...
Module(
 {
  ...
  type: 'search',
  info: 'Sends info of given query from Google Search.',
  use: '<text>',
 },
 async (Suhail, citel, text) => {
 ...
 async (Suhail, citel, text) => {
  ...
  try {
   ...
   for (let url of urlsArray) {
    Suhail.bot.sendMessage(citel.chat, { image: { url: url } })
   }
  } catch (error) {
   return citel.reply('*Google Images Not Working, Try it Later*')
  }
...
Module(
 {
  ...
  type: 'search',
  info: 'Searches in given rage about given number.',
  use: '9112345678xx',
 },
 async (Suhail, citel, text) => {
  ...
  for (let i = 0; i < randomxx; i++) {
   ...
   try {
    try {
     var anu1 = await Suhail.bot.fetchStatus(anu[0].jid)
    } catch {
     var anu1 = '401'
    }

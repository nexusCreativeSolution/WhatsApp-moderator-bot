const { isPrivate, command, isAdmin } = require('../../lib')
const { setMute, getMute } = require('../database/mute')
const { addTask } = require('../../lib/mute')
const { c24to12 } = require('../../lib/functions')
bot(
 {
  pattern: 'amute ?(.*)',
  fromMe: isPrivate,
 ...
 async (message, match) => {
  ...
  const anAdmin = await isAdmin(participants, message.client.user.jid)
  if (!anAdmin) return await message.send(`_I'm not admin._`)
  let msg = message.reply_message.text || 'null'
  const [hour, mins] = match.split(' ')
  if (hour == 'info') {
   ...
   return await message.send(`*Hour :* ${hour}\n*Minute :* ${minute}\n*Time :* ${c24to12(`${hour}:${minute}`)}\n*Mute :* ${enabled ? 'on' : 'off'}\nMessage : ${msg}`)
  }
  if (hour == 'on' || hour == 'off') {
   ...
   const isTask = addTask(message.jid, 'mute', hour == 'off' ? 'off' : task.hour, task.minute, task.msg)
  ...
  if (!hour || !mins || isNaN(hour) || isNaN(mins)) return await message.send('*Example : amute 6 0*\namute on | off\namute list\nReply to a text to send Msg on mute')
  await setMute(message.jid, 'mute', true, hour, mins, msg)
  addTask(message.jid, 'mute', hour, mins, msg)

  return await message.send(`_Group will Mute at ${c24to12(`${hour}:${mins}`)}_${msg != 'null' ? `\n_Message: ${msg}_` : ''}`)
 }
)
...
bot(
 ...
 async (message, match) => {
  ...
  if (hour == 'on' || hour == 'off') {
   ...
   const isTask = addTask(message.jid, 'unmute', hour == 'off' ? 'off' : task.hour, task.minute, task.msg)
  ...
  if (!hour || !min || isNaN(hour) || isNaN(min)) return await message.send('*Example : aunmute 16 30*\naunmute on | off\naunmute list\nReply to a text to send Msg on unmute')
  await setMute(message.jid, 'unmute', true, hour, min, msg)
  addTask(message.jid, 'unmute', hour, min, msg)
  return await message.send(`_Group will unMute at ${c24to12(`${hour}:${min}`)}_${msg != 'null' ? `\n_Message: ${msg}_` : ''}`)
 }
)

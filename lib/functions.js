...
const { jidDecode, delay, generateWAMessageFromContent, proto } = require('@whiskeysockets/baileys')
...
const { loadMessage } = require('../resources/database/StoreDb')
 ...
 * @param {string} time24 - A time string in 24-hour format (e.g., "23:45").
 * @returns {string} A time string in 12-hour format (e.g., "11:45 PM").
 */
async function c24to12(time24) {
 const [hour24, minute] = time24.split(':').map(Number)
 const period = hour24 >= 12 ? 'PM' : 'AM'
 const hour12 = hour24 % 12 || 12 // Converts '0' hour to '12'
...
module.exports = {
 ...
 AddMp3Meta: async (songbuffer, coverBuffer, options = { title: 'fx-bot Whatsapp bot', artist: ['Xasena'] }) => {
  ...
  writer.setFrame('TIT2', options.title).setFrame('TPE1', ['fx-bot']).setFrame('APIC', {
   type: 3,
   data: coverBuffer,
   description: 'Xasena',
  })

...
const { default: makeWASocket, useMultiFileAuthState, fetchLatestBaileysVersion, Browsers, delay, makeCacheableSignalKeyStore, DisconnectReason } = require('@whiskeysockets/baileys')
const { PausedChats } = require('../resources/database')
const util = require('util')
const config = require('../config')
const { serialize, Greetings } = require('./index')
const { Image, Message, Sticker, Video, AllMessage } = require('./Messages')

const { loadMessage, saveMessage, saveChat, getName } = require('../resources/database/StoreDb')
const { MultiFileAuthState } = require('./auth')

const logger = pino({ level: 'silent' })
const connect = async () => {
 const sessionDir = './database.db'
 if (!fs.existsSync(sessionDir)) fs.mkdirSync(sessionDir)

 const { state, saveCreds } = await MultiFileAuthState(path.join(__basedir, sessionDir))
 ...
 process.on('unhandledRejection', async err => {
  ...
  const text = `\`\`\`fx-bot unhandledRejection: \n${error}\`\`\``
  await conn.sendMessage(conn.user.id, { text })
  console.log(err)
 })
 process.on('uncaughtException', async err => {
  const error = util.format(err)
  const text = `\`\`\`fx-bot uncaughtException: \n${error}\`\`\``
  await conn.sendMessage(conn.user.id, { text })
  console.log(err)
 })
 return conn
...
const handleConnectionUpdate = conn => async s => {
 ...
 if (connection === 'connecting') console.log('Connecting to WhatsApp... Please Wait.')
  ...
  const connectedMsg = `\`\`\`fx-bot connected\nVersion: ${packageVersion}\nPlugins: ${totalPlugins}\nWorktype: ${workType}\`\`\``
  conn.sendMessage(conn.user.id, { text: connectedMsg })
...
const handleMessages = conn => async m => {
 ...
 plugins.commands.map(async command => {
  ...
  const handleCommand = (Instance, args) => {
   whas = new Instance(conn, msg)
   command.function(whas, ...args, msg, conn, m)
  }
   ...
   switch (command.on) {
    ...
    case 'delete':
     if (msg.type === 'protocolMessage') {
      whas = new Message(conn, msg)
      whas.messageId = msg.message.protocolMessage.key?.id
      command.function(whas, msg, conn, m)
     }
     break
    case 'message':

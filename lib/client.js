...
const handleMessages = conn => async m => {
 ...
 plugins.commands.map(async command => {
  ...
  const handleCommand = (Instance, args) => {
   wabot = new Instance(conn, msg)
   command.function(wabot, ...args, msg, conn, m)
  }
   ...
   switch (command.on) {
    ...
    case 'delete':
     if (msg.type === 'protocolMessage') {
      wabot = new Message(conn, msg)
      wabot.messageId = msg.message.protocolMessage.key?.id
      command.function(wabot, msg, conn, m)
     }
     break
    case 'message':

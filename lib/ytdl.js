...
const search = async (query, limit = 1) => {
 ...
 return searchResults.items.map(({ title, url, author, views, duration, uploadedAt }) => {
...
async function ytdlDl(media, input) {
 const data = `vid=${media}&k=${encodeURIComponent(input)}`
